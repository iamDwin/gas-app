...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, OrganizationFormComponent, DataTableComponent, ButtonComponent],
  template: `
    <div class="p-4">
      <app-data-table
        [rows]="organizations"
...
export class OrganizationsComponent implements OnInit {
  ...
  onActionClick(event: {action: TableAction, row: Organization}) {
    switch (event.action.label) {
      case 'Edit':
        ...
        break;
      case 'Delete':
        this.deleteOrganization(event.row.id);
        break;
    }
  }
  ...
  saveOrganization(orgData: CreateOrganizationRequest) {
    this.loadingService.show(this.selectedOrganization ? 'Updating organization...' : 'Creating organization...');
