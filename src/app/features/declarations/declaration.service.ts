...
import { Declaration, DeclarationApproval, DeclarationStatus } from './declaration.model';
...
export class DeclarationService {
  ...
  getDeclarationsForOrganization(organizationId: string): Observable<Declaration[]> {
    return new Observable(subscriber => {
      const filtered = this.declarations.value.filter(d => d.organizationId === organizationId);
  ...
  getPendingApprovals(): Observable<Declaration[]> {
    ...
    return new Observable(subscriber => {
      ...
      if (this.authService.isAdmin()) {
        filtered = this.declarations.value.filter(d => 
          d.status === 'pending_admin_approval'
        );
  ...
  addDeclaration(declaration: Omit<Declaration, 'id' | 'createdAt' | 'updatedAt' | 'status'>): void {
    const user = this.authService.getCurrentUser();
  ...
  approveDeclaration(declarationId: string, comments?: string): void {
    ...
    const declaration = this.declarations.value.find(d => d.id === declarationId);
    ...
    const updatedDeclaration = {
      ...declaration,
      status: this.authService.isAdmin() ? 'approved' as DeclarationStatus : 'pending_admin_approval' as DeclarationStatus,
      updatedAt: new Date(),
      approvedBy: user.id,
      approvedAt: new Date()
    };
