...
export class ConfirmationModalComponent {
  ...
  get overlayClass(): string {
    return this.config.type === 'danger' ? 'bg-red-100/10' : 
           this.config.type === 'warning' ? 'bg-yellow-100/10' : 
           'bg-blue-100/10';
  }
  ...
  get iconContainerClass(): string {
    ...
    return this.config.type === 'danger' ? `${baseClass} bg-red-100` :
           this.config.type === 'warning' ? `${baseClass} bg-yellow-100` :
           `${baseClass} bg-blue-100`;
  }

  get confirmButtonClass(): string {
    const baseClass = 'inline-flex w-full justify-center rounded-md px-3 py-2 text-sm font-semibold shadow-sm sm:ml-3 sm:w-auto';
